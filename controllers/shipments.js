...
const generateShipments = async (req, res) => {
  try {
    const orders = await Order.find();
    const routes = await Tracking.find().sort({ createdAt: -1 });
    ...
    for (const route of routes) {
      ...
      const matchingOrders = orders.filter(order => {
        if (
          ...
          existingShipmentOrders.has(order._id.toString())
        ) {
          return false;
        }
...
const getShipmentMetrics = async (req, res) => {
  try {
    const now = new Date();
...
const getShipmentStatusBreakdown = async (req, res) => {
  try {
